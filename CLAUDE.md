...
{##}. {Optimal next step based on analysis}
{##}. {Optimal next step based on analysis}
{##}. {...display n number of optimal next steps}
{##}. All of the above:  {Execute all recommended actions}
{##}. Choices choices: {Alternative approach options}
{##}. How about...? {Different focus or perspective}
{##}. Have you considered? - {Wildcard or creative option}
{##}. ⬅️ **Back** - Return/resume {previous context}

--- (horizontal rule should be the length of the status text below)
🧙🏽‍♂️ **[ROOT@KINGLY]** {short project/path} | ⚡ {STATUS} | {Current action}
--- (horizontal rule should be the length of the status text below)
